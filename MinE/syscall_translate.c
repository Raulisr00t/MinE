...
static int64_t sys_read(int fd, void* buf, uint64_t count)
{
    ...
    if (!h) return -(int64_t)LINUX_EBADF;
    DWORD got = 0;
    if (!ReadFile(h, buf, (DWORD)count, &got, NULL))
...
static int64_t sys_writev(int fd, const Linux_iovec* iov, int iovcnt)
{
    ...
    for (int i = 0; i < iovcnt; i++) {
        int64_t n = sys_write(fd, (void*)iov[i].iov_base, iov[i].iov_len);
    ...
    uint32_t flags, int fd, uint64_t off)
{
    ...
    if (wprot) { DWORD old; VirtualProtect(p, (SIZE_T)alen, wprot, &old); }
...
static int64_t sys_getcwd(char* buf, uint64_t size)
{
    ...
    if (len + 1 > size) return -(int64_t)LINUX_EINVAL;
